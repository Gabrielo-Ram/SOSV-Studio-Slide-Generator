 ...
 */
export async function parseCSVStream(filePath: string): Promise<Startup[]> {
  try {
    ...
    await asyncPipeline(
      fs.createReadStream(filePath), //Reads raw CSV
      parse({
...
async function testFunction() {
  try {
    ...
    if (process.argv.length < 3) {
      console.error(
        "Improper usage: Please provide a CSV file path: node ./build/parseCSV.js <file.csv>"
      );
      process.exit(1);
    }
