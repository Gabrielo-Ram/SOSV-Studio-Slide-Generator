...
const KEY_PATH = path.resolve(__dirname, "../serviceCredentials/service-account-key.json");
 ...
 */
async function makePresentationPublic(presentationID, drive) {
    try {
        ...
        throw new Error(`Could not change permissions for presentation: \n${error}`);
    }
}
/**
 ...
 * @param {drive_v3.Drive} drive The Drive Client instance
 */
async function deleteAllOwnedFiles(drive) {
    ...
    console.log(`Found ${files.length} ${files.length === 1 ? "file" : "files"}. Deleting...`);
